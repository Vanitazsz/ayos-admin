...
-- WORKER accounts in a single request. Mirrors the customer bulk RPCs
-- (admin_bulk_set_account_status / admin_bulk_set_customer_verification)
-- but scoped to accounts.role = 'WORKER'. Worker "verification" is modeled
-- as worker_profiles.approval_status, so Verify/Unverify overrides it the
-- same way admin_set_customer_verification overrides user_profiles.
...
begin
  ...
  if affected is not null then
    ...
    insert into public.audit_logs(actor_id, action, entity_type, entity_id, metadata)
    select auth.uid(), 'WORKER_VERIFICATION_STATUS_UPDATED', 'account',
