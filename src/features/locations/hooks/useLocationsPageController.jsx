import { loadLocations, saveLocation } from '../logic/LocationsPageLogic';
...
export function useLocationsPageController() {
  const { data: rows, isLoading, error, refresh } = useDataFetch(loadLocations, []);
  const safeRows = rows ?? [];
  ...
  const submit = async (event) => {
    ...
    try {
      ...
      setFormError(reason instanceof Error ? reason.message : 'Unable to save location');
    } finally {
      setSaving(false);
    }
  };
  const toggle = async (row) => {
    setConfirm({
      isOpen: true,
      title: row.is_active ? 'Deactivate Location' : 'Activate Location',
      message: `${row.is_active ? 'Deactivate' : 'Activate'} ${row.name}?`,
      onConfirm: async () => {
        await saveLocation({ ...row, is_active: !row.is_active });
        await refresh();
      },
    });
  };
