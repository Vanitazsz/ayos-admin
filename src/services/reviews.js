...
const REVIEW_SELECT =
  'id,booking_id,user_account_id,worker_account_id,stars,body,recommend_worker,moderation_status,moderated_by,moderated_at,created_at,updated_at,user_profiles:user_account_id(display_name),worker_profiles:worker_account_id(display_name),bookings(id,status,created_at,booking_proof_media(storage_path,content_type),service_requests(description,scheduled_at,addresses(line1,barangay,city),service_categories(name))),review_media(storage_path,content_type)';
...
export const mapReview = (row) => {
  ...
  return {
    ...
    proofMedia: dedupeByPath(
      (row.bookings?.booking_proof_media ?? []).map((item) => ({
        path: item.storage_path,
        contentType: item.content_type,
      })),
    ),
...
export async function resolveReviewMedia(review) {
  ...
  const sign = async (bucket) => {
    const paths = items.map((item) => item.path).filter((path) => !urlByPath.has(path));
  ...
  await sign('review-media');
  await sign('booking-proof');
